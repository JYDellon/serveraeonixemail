...
// Route de test pour vérifier que le serveur fonctionne
app.get('/', (req, res) => {
  res.send('Serveur opérationnel. Utilise l\'endpoint /api/devis ou /api/contact pour envoyer un email.');
});

// Configuration du transporteur pour l'envoi d'email
const transporter = nodemailer.createTransport({
  service: 'gmail', // Utilise Gmail pour l'envoi d'email
  auth: {
    user: process.env.EMAIL_USER, // Ton adresse email Gmail
    pass: process.env.EMAIL_PASS, // Utilisation du mot de passe d'application ici
  },
});
...
app.post('/api/devis', (req, res) => {
  ...
  const mailOptions = {
    from: process.env.EMAIL_USER, // L'email depuis lequel l'email sera envoyé
    to: process.env.EMAIL_DESTINATAIRE, // Email du destinataire
    subject: `Demande de devis pour ${typeProjet}`,
    text: `
      Demande de devis pour un projet ${typeProjet} :
